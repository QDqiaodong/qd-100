    ...
    <div class="bg-gradient-to-r from-orange-500 via-primary to-red-500 rounded-2xl p-6 text-white shadow-lg">
      ...
      <div v-else class="bg-white/10 backdrop-blur rounded-xl p-4 min-h-48">
        <div v-show="activeTab === 'tags'">
          <div v-if="report.hotTags.length === 0" class="h-40 flex flex-col items-center justify-center text-white/60">
            <Flame class="w-10 h-10 mb-2 opacity-50" />
            <p class="text-sm">暂无热门标签</p>
          </div>
          <div v-else class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            >
              <div class="flex items-center gap-2 mb-2">
                ...
                <span v-if="tag.trend === 'up'" class="flex items-center text-red-300">
                  <TrendingUp class="w-4 h-4" />
                </span>
                <span v-else-if="tag.trend === 'stable'" class="flex items-center text-yellow-300">
                  <Flame class="w-4 h-4" />
                </span>
                <span v-else class="flex items-center text-white/40">
        ...
        <div v-show="activeTab === 'authors'">
          <div v-if="report.newAuthors.length === 0" class="h-40 flex flex-col items-center justify-center text-white/60">
            <Users class="w-10 h-10 mb-2 opacity-50" />
            <p class="text-sm">暂无新锐作者</p>
          </div>
        ...
        <div v-show="activeTab === 'videos'">
          <div v-if="report.trendingVideos.length === 0" class="h-40 flex flex-col items-center justify-center text-white/60">
            <TrendingUp class="w-10 h-10 mb-2 opacity-50" />
            <p class="text-sm">暂无涨势视频</p>
          </div>
          <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            >
              <div class="relative aspect-video bg-black/30">
                ...
                <div 
                  class="absolute top-2 left-2 px-2 py-1 rounded-full text-xs font-bold flex items-center gap-1"
                  :class="video.growthRate >= 70 ? 'bg-red-500' : video.growthRate >= 40 ? 'bg-orange-500' : 'bg-green-500'"
                >
